...
class BuildingInstanceRepository(BaseRepository[BuildingInstanceEntityPydantic, BuildingInstanceDB, UUID]):
    """
    Repository for BuildingInstanceDB data operations.
    """
    def __init__(self, db: AsyncSession):
        super().__init__(db=db, model_cls=BuildingInstanceDB, entity_cls=BuildingInstanceEntityPydantic)
        logging.info(
            f"BuildingInstanceRepository initialized with model: {self.model_cls.__name__} "
            f"and entity: {self.entity_cls.__name__}"
        )

    async def find_by_settlement_id(self, settlement_id: UUID, skip: int = 0, limit: int = 100) -> List[BuildingInstanceEntityPydantic]:
    ...
    async def find_by_blueprint_id(self, blueprint_id: UUID, skip: int = 0, limit: int = 100) -> List[BuildingInstanceEntityPydantic]:
    ...
    async def get_instance_with_details(self, instance_id: UUID) -> Optional[BuildingInstanceEntityPydantic]:
