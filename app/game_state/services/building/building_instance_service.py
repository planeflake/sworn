...
from app.game_state.entities.building.building_instance_pydantic import BuildingInstanceEntityPydantic, BuildingStatus
...
# NOTE: BuildingBlueprintEntity import removed - now using Pydantic entities
...
class BuildingInstanceService:
    ...
    async def _convert_entity_to_read_schema(self, entity: Optional[BuildingInstanceEntityPydantic]) -> Optional[BuildingInstanceRead]:
        if entity is None:
            return None
        try:
            # Use to_dict for proper entity to API schema conversion
    ...
    async def update_building_instance(self, instance_id: uuid.UUID, update_data: BuildingInstanceUpdate) -> Optional[BuildingInstanceRead]:
        ...
        try:
            updated_entity = await self.repository.update_entity(instance_id, update_data)
            if updated_entity:
                return await self._convert_entity_to_read_schema(updated_entity)
            else:
                logging.warning(f"Building instance not found for update: {instance_id}")
                return None
        except Exception as e:
            logging.error(f"Database error updating building instance ID {instance_id}: {e}", exc_info=True)
            raise ValueError(f"Could not update building instance: {e}") from e
